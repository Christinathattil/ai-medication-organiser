-- ========================================
-- FINAL RLS FIX: Corrected for Actual Schema
...
-- INTERACTIONS: Between TWO medications (medication1_id and medication2_id)
-- User can see interaction if they own EITHER medication
CREATE POLICY "interactions_select_own"
  ON interactions FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM medications
      WHERE medications.id = interactions.medication1_id
      AND medications.user_id = current_setting('app.current_user_id', TRUE)::bigint
    )
    OR
    EXISTS (
      SELECT 1 FROM medications
      WHERE medications.id = interactions.medication2_id
      AND medications.user_id = current_setting('app.current_user_id', TRUE)::bigint
    )
  );

-- User can add interaction if they own BOTH medications
...
-- User can update interaction if they own either medication
...
-- User can delete interaction if they own either medication
...
BEGIN 
  ...
  RAISE NOTICE '  - users (own profile only)';
  RAISE NOTICE '  - medications (own meds only)';
  RAISE NOTICE '  - schedules (via medications)';
  RAISE NOTICE '  - medication_logs (via schedules)';
  RAISE NOTICE '  - interactions (via medication1_id & medication2_id)';
  RAISE NOTICE '  - sms_reminders (via medication_id)';
  RAISE NOTICE '';
